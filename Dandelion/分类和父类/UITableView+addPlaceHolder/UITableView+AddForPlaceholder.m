...
- (void)layoutDefaultView:(UIView *)defaultView {
    ...
    UIImage *image = self.defaultNoDataImage ? : [UIImage imageNamed:@"zanwu_sj"];
    imageView.image = image;
    CGFloat X = (self.bounds.size.width - image.size.width - self.offSetX - self.contentInset.left - self.contentInset.right) / 2;
    CGFloat Y = (self.bounds.size.height - image.size.height - self.offSetY - self.contentInset.top - self.contentInset.bottom) / 3;
...
- (UILabel *)defaultNoDataNoticeLabel {
    ...
    if([className isEqualToString:@"BJLChatViewController"]){
        ...
        if (!label) {
            ...
            //label.text = self.defaultNoDataText ? : @"空空如也";
...
//            imageView.image = self.defaultNoDataImage ? : [UIImage imageNamed:@"icon_wsj.png"];
            imageView.image = self.defaultNoDataImage ? : [UIImage imageNamed:@"zanwu_sj"];
...
- (CGFloat)offSetX {
    ...
    return [objc_getAssociatedObject(self, @"offSetX") floatValue];
}

- (void)setOffSetX:(CGFloat)offSetX {
    objc_setAssociatedObject(self, @"offSetX", @(offSetX), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (CGFloat)offSetY {
    
    return [objc_getAssociatedObject(self, @"offSetY") floatValue];
}

- (void)setOffSetY:(CGFloat)offSetY {
    objc_setAssociatedObject(self, @"offSetY", @(offSetY), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}
