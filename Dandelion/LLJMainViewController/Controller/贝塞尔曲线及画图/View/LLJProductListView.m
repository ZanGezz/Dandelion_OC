...
- (void)reloadData:(NSArray *)sourceArray {
    ...
    //是否数据过多，导致一列无法显示
    BOOL r = (self.dataArray.count *Lable_H + (self.dataArray.count - 1)*Offset_Y) > self.frame.size.height ? YES : NO;
    //重新计算设置view的frame
    CGFloat W,H;
    //计算创建行数和列数
    int row = 0, line = 0;
    if (r) {
        line = (int)self.dataArray.count/row;
        row = (self.frame.size.height - Lable_H) / (self.dataArray.count - 1)*(Offset_Y + Lable_H) + 1;
        W = line *View_W;
        H = (Offset_Y + Lable_H)*row + Lable_H;

    } else {
        line = 1;
        row = (int)self.dataArray.count;
        W = View_W;
        H = (Offset_Y + Lable_H)*self.dataArray.count - Offset_Y;
    }
    ...
    for (int i = 0; i < line; i ++) {
        ...
        for (int j = 0; j < row; j ++) {
            ...
            subView.frame = CGRectMake(0, (Offset_Y + Lable_H)*j, View_W, Lable_H);
