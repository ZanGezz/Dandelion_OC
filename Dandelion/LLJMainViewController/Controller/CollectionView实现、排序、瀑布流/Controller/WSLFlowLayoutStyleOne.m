...
- (void)viewDidLoad {
    ...
    UICollectionView * collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 64) collectionViewLayout:_flow];
    ...
    //注册Cell，必须要有
    [collectionView registerClass:[LLJChanelItem class] forCellWithReuseIdentifier:@"ItemID"];
...
- (void)refreshLayout{
    ...
    if (_flow.flowLayoutStyle == (WSLWaterFlowLayoutStyle)3){
        ...
        _flow.collectionView.frame = CGRectMake(0, 0, self.view.frame.size.width, average * 2 + [self rowMarginInWaterFlowLayout:_flow] + [self edgeInsetInWaterFlowLayout:_flow].top + [self edgeInsetInWaterFlowLayout:_flow].bottom);
    }else{
        _flow.collectionView.frame = CGRectMake(0, 0, self.view.frame.size.width, self.view.frame.size.height - 64);
    }
...
- (CGSize)waterFlowLayout:(WSLWaterFlowLayout *)waterFlowLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath{
    if(waterFlowLayout.flowLayoutStyle == (WSLWaterFlowLayoutStyle)0){
        return CGSizeMake(0, [_array[indexPath.row] floatValue]);
    }else if (waterFlowLayout.flowLayoutStyle == (WSLWaterFlowLayoutStyle)1){
        return CGSizeMake([_array[indexPath.row] floatValue], 0);
    }else if (waterFlowLayout.flowLayoutStyle == (WSLWaterFlowLayoutStyle)2){
        return CGSizeMake([_array[indexPath.row] floatValue], 100);
...
{
    LLJChanelItem *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"ItemID" forIndexPath:indexPath];
    ...
    [cell setIamgeName:@"" index:indexPath.row deletHidden:NO];
    
    cell.backgroundColor = [UIColor arc4randomColor:1];
