...
- (void)createVideoPlayer {
    ...
    LLJWeakSelf(self);
    //返回
    self.videoPlayer.defaultUI.backClick = ^(UIButton * _Nonnull sender) {
        NSLog(@"返回2");
        if (weakSelf.videoPlayer.isCurrentFullScreen) {
            [weakSelf.videoPlayer.rotateManage viewRotate];
        }else{
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [self.videoPlayer exchangePlayerSourceWithModel:[self createPlayModelWithURL:self.imageArray[indexPath.row]]];
}
