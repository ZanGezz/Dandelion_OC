...
@implementation LLJTopControlView

- (instancetype)initWithPlayerModel:(LLJVideoPlayModel *)model {
    self = [super initWithPlayerModel:model type:LLJControlViewTypeTop];
    if (self) {
        //设置颜色渐变
        ...
        case LLJAddsubViewTypeLeftOrTop:
        {
            ...
            for (int i = 0; i < self.leftOrTopSubViewArray.count; i ++) {
                ...
                if (model.subView) {
                    ...
                    [self setUpanimateHiddenSmallScreenFrame:model type:LLJControlViewTypeTop];
                }
            }
        }
            break;
