...
- (void)createDefaultPlayerUI {
    ...
    self.defaultUI.sliderEnd = ^(UISlider * _Nonnull sender) {
        ...
        [weakSelf.playManage llj_seekToTime:weakSelf.playManage.totalTime*sender.value completionHandler:^(BOOL finished) {
        }];
    };
        ...
        switch (type) {
            ...
            case LLJGesClickTypePanHorizontal:
            {
                switch (state) {
                    ...
                    {
                        //快进
                        [weakSelf.playManage llj_seekToTime:weakSelf.playManage.totalTime*weakSelf.defaultUI.sliderValue completionHandler:^(BOOL finished) {
                        }];
                    }
                        break;
