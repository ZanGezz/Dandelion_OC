...
- (void)actionSheetWithTakePhotoTitle:(NSString *)takePhotoTitle  albumTitle:(NSString *)albumTitle cancelTitle:(NSString *)cancelTitle InViewController:(UIViewController *)viewController didFinished:(imagePickerDidFinished)finished didCancel:(imagePickerDidCancel)cancel{
    ...
    if (takePhotoTitle.length) {
        UIAlertAction *action1 = [UIAlertAction actionWithTitle: takePhotoTitle style: UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
            //处理点击拍照
        ...
        [action1 setValue:LLJColor(246, 100, 100, 1) forKey:@"titleTextColor"];
        [alertController addAction:action1];
    }
    if (albumTitle.length) {
        
        UIAlertAction *action2 = [UIAlertAction actionWithTitle: albumTitle style: UIAlertActionStyleDefault handler:^(UIAlertAction *action){
            //处理点击从相册选取
        ...
        [action2 setValue:LLJColor(246, 100, 100, 1) forKey:@"titleTextColor"];
    ...
    if (cancelTitle.length) {
        ...
        [actionCancel setValue:LLJColor(246, 100, 100, 1) forKey:@"titleTextColor"];
