...
- (void)setupPickerView {
    ...
    if (self.isSelectGender) {
        self.genderString = self.yearList[0];
    }
}
...
- (void)selectedDateLogic {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(datePicker:didSelectDate:gender:)]) {
        [self.delegate datePicker:self didSelectDate:self.selectedComponents gender:self.genderString];
    }
...
- (void)pickerView:(PGPickerView *)pickerView title:(NSString *)title didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    ...
    if (self.isSelectGender) {
        self.genderString = self.yearList[row - 1];
    }
...
- (void)setAssignDate:(NSDate *)assignDate{
    _setDate = assignDate;
}
- (NSCalendar *)calendar {
    if (!_calendar) {
