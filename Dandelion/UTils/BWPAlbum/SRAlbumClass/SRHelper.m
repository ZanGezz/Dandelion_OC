...
+ (NSString *)compressedVideoPath{
    ...
    NSString *fileName = [[LLJ_video_Path stringByAppendingPathComponent:nowTimeStr] stringByAppendingString:@".mp4"];
    return fileName;
}
...
+(BOOL)deleteFileByPath:(NSURL *)path{
    if (!path) {
        path = [NSURL URLWithString:LLJ_video_Path];
    }
    NSFileManager* fileManager=[NSFileManager defaultManager];
...
+(BOOL)deleteAllFile{
    ...
    NSURL *path = [NSURL URLWithString:LLJ_video_Path];
    NSFileManager* fileManager=[NSFileManager defaultManager];
    BOOL blHave=[fileManager fileExistsAtPath:[path path]];
    if (blHave) {
        blHave = [fileManager removeItemAtURL:path error:nil];
