...
- (void)loadAlbumData{
    if (self.assetCollection) {
        ...
        [SRHelper fetchAlbumsOption:option contentBlock:^(PHFetchResult * _Nonnull content, BOOL success) {
...
- (void)configerView{
    ...
    }else if(self.assetType == SRAssetTypeVideo){
        self.title = self.assetCollection == nil?@"所有视频":self.assetCollection.localizedTitle;
    }
...
//    [self.selecteds removeAllObjects];
//    for (PHAsset *phAsset in self.fetchResult) {
//        [self.selecteds addObject:phAsset];
//    }
    [self finishAsset];
}
