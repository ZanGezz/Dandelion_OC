...
@property (nonatomic, strong) PAMZTimerLabel                *timeLabel;

@property (nonatomic, copy) NSString                        *vidoPath;
...
- (void)configerView{
    ...
    if ([SRAlbumConfiger singleton].assetType == SRAssetTypePic) {
        ...
        self.nextStepButton.backgroundColor = LLJColor(246, 101, 101, 1);
        self.nextStepButton.layer.masksToBounds = YES;
        self.nextStepButton.layer.cornerRadius = 15.0f;
    }else if([SRAlbumConfiger singleton].assetType == SRAssetTypeVideo){
        self.nextStepButton.hidden = YES;
        self.timeLabel = [[PAMZTimerLabel alloc]initWithLabel:self.videoTimeLabel andTimerType:MZTimerLabelTypeStopWatch];
    }
    
    self.titleLabel.layer.shadowColor = [UIColor blackColor].CGColor;
    self.titleLabel.layer.shadowOffset = CGSizeMake(0,0);
    self.titleLabel.text = [SRAlbumConfiger singleton].assetType == SRAssetTypeNone?@"轻触拍照，点击摄像":([SRAlbumConfiger singleton].assetType == SRAssetTypePic?@"轻触拍照":@"点击摄像");
}
...
- (IBAction)didClickDismiss:(UIButton *)sender {
    ...
    [SRHelper deleteFileByPath:[NSURL URLWithString:self.vidoPath]];
}
    ...
    if (self.motionManager.deviceMotionAvailable) {
        ...
        [self.motionManager startDeviceMotionUpdatesToQueue:[NSOperationQueue mainQueue] withHandler:^(CMDeviceMotion *motion, NSError *error) {
            if (!error) {
                ...
                if (direction != weakSelf.screenDirection) {
                    weakSelf.screenDirection = direction;
                }
            }
        }];
    }else{//不可以使用则释放
...
- (NSString *)getVideoSaveFilePathString{
    ...
    if (![[NSFileManager defaultManager] fileExistsAtPath:LLJ_Base_Path]) {
        [[NSFileManager defaultManager] createDirectoryAtURL:[NSURL fileURLWithPath:LLJ_Base_Path] withIntermediateDirectories:YES attributes:nil error:nil];
    }
    ...
    NSString *filePath = [LLJ_Base_Path stringByAppendingPathComponent:fileName];
    return filePath;
}
...
- (void)shutterCamera{
    ...
    if (self.photoArray.count == 9) {
        [MBProgressHUD showMessag:@"最多拍摄9张" toView:kRootView hudModel:MBProgressHUDModeText hide:YES];
        return;
    }
    ...
    if (@available(iOS 10.0,*)) {
        ...
        if (videoConnection) {
            videoConnection.videoOrientation = [self getCaptureVideoOrientation];
        ...
        if (videoConnection) {
            ...
            [self.stillImageToOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageDataSampleBuffer, NSError *error) {
                ...
                [weakSelf updateNextStepBtn:image];
//                self.showView.hidden = NO;
//                self.showImage.image = image;
                weakSelf.content = image;
            }];
        }
    }
}
/**
 TODO:收集pic更新按钮名称
 */
- (void)updateNextStepBtn:(UIImage *)image{
    
    //收集拍摄的照片
    [self.photoArray addObject:image];
    //更新下一步按钮
    NSString *nextStep = [NSString stringWithFormat:@"下一步(%lu)",(unsigned long)self.photoArray.count];
    [self.nextStepButton setTitle:nextStep forState:UIControlStateNormal];
}
- (IBAction)nextStep:(UIButton *)sender {
    if (!IS_VALID_ARRAY(self.photoArray)) {
        [MBProgressHUD showMessag:@"您还没有拍摄照片" toView:kRootView hudModel:MBProgressHUDModeText hide:YES];
    }else{
...
- (void)stopCurrentVideoRecording{
    ...
    self.cameraButton.effectview.image = [UIImage imageNamed:@"video_pai01"];
    self.currentTime = 0;
...
- (void)didClickCameraButtonIsVideotape:(BOOL)isVideotape isStart:(BOOL)isStart{
    if (!isVideotape && [SRAlbumConfiger singleton].assetType != SRAssetTypeVideo) {//拍照
        ...
        if ([SRAlbumConfiger singleton].assetType != SRAssetTypePic) {
            if (isStart) {
                self.vidoPath = [self getVideoSaveFilePathString];
                [self startRecordingToOutputFileURL:[NSURL fileURLWithPath:self.vidoPath]];
            }else{
                [self stopCurrentVideoRecording];
            }
        }
